...
3.Place Gaze_Following.py in the root directory of K230's sdcard and rename it to main.py, then reset or power on.
...
After K230 is initialized successfully, it will start to recognize the direction of your gaze. When you look to the right while facing the car, the car will also rotate to your right.
...
GazeDire_Track: Calculates the deviation between the starting and ending positions of the gaze direction to control the car's rotation in situ, and limits the output value of pid and the car's rotation range to prevent the car from being unable to continue to identify the gaze direction due to excessive rotation.
