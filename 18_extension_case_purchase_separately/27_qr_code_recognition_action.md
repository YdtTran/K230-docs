# 6.QR code recognition action

# QR code recognition action
...
![image-20250603170101160](https://www.yahboom.com/public/upload/upload-html/1749210844/image-20250603170101160.png)
...
Open the ProtocolHub project used by STM32, first check the beginning of the main.c file, the first thing to do is to change the number of MOTOR_TYPE to the motor you are using, after the modification is correct, compile and burn, and finally reset or power on.

3.Place QR_code_rec_action.py in the root directory of K230's sdcard and rename it to main.py, then reset or power on.

If you are not sure how to burn the program into K230, please read the K230 Quick Start tutorial first.

1. 

Move the K230 module bracket to a suitable angle and connect the power supply.
2. 

After K230 is initialized successfully, it will start to recognize the content of the QR code and take corresponding actions based on the content.

 

##### Notice:

The QR code action currently only responds to the following six commands: "FORWARD", "BACKWARD", "LEFT", "RIGHT", "ROTATE LEFT", and "ROTATE RIGHT". All uppercase or all lowercase characters are supported.
...
void QRCode_Action(const char* msg)
{
 ...
    {
        DEBUG_PRINT("FORWARD!\r\n");
        Motion_Car_Control(200,0,0);//前进两秒后停下    Go forward for two seconds and then stop
        delay_ms(1000);
        delay_ms(1000);
...
QRCode_Action: When the content of the QR code is recognized as "FORWARD", it moves forward for two seconds. When it is recognized as "BACKWARD", it moves back for two seconds. "LEFT" means turn left for two seconds, "RIGHT" means turn right for two seconds, "ROTATE LEFT" means turn left for two seconds, and "ROTATE RIGHT" means turn right for two seconds.
...
![qrcode](https://www.yahboom.com/public/upload/upload-html/1749210844/qrcode.png)