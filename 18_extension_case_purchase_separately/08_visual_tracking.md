...
Open the ProtocolHub project used by MSPM0G3507, first check the beginning of the main.c file. The first thing to do is to change the number of MOTOR_TYPE to the motor you are using, and the second is to change color_mode to 1, which turns on the line patrol mode. After these two places are modified correctly, compile and burn, and finally reset or power on again.
