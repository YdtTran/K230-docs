...
![image-20250603170101160](https://www.yahboom.com/public/upload/upload-html/1749210871/image-20250603170101160.png)
...
Open the ProtocolHub project used by STM32, first check the beginning of the main.c file. The first thing to do is to change the number of MOTOR_TYPE to the motor you are using, and the second is to change Car_Auto_Drive to 1 to turn on the mode of autonomous forward movement of the car after power-on. After these two places are modified correctly, compile and burn, and finally reset or power on again.
3. 

Place autonomous_avoidance.py in the root directory of K230's sdcard and rename it to main.py, then reset or power on.

If you are not sure how to burn the program into K230, please read the K230 Quick Start tutorial first.

1. 

Place the car on the ground, move the K230 module bracket to a suitable angle, and connect the power supply.
2. 

After powering on, the car will wait for a few seconds until the K230 is initialized. Then the car will start to move forward by itself. When it encounters an ice cream bucket, it will identify it as an obstacle and stop moving. When the ice cream bucket is removed, the car will continue to move forward.
...
Autonomous_Avoid: When an obstacle is detected and approaches an area larger than the set target_area, the car will stop.
...
![image-20250603193526470](https://www.yahboom.com/public/upload/upload-html/1749210871/image-20250603193526470.png)
