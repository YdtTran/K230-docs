...
![图层-3 (10)](https://www.yahboom.com/public/upload/upload-html/1749210422/%E5%9B%BE%E5%B1%82-3%20(10).png)
...
Open the ProtocolHub project used by MSPM0G3507, first check the beginning of the main.c file. The first thing to do is to change the number of MOTOR_TYPE to the motor you are using, and the second is to change color_mode to 2 to turn on the color tracking mode. After these two places are modified correctly, compile and burn, and finally reset or power on again.
3. 

Place color_track.py in the root directory of K230's sdcard and rename it to main.py, then reset or power on.

If you are not sure how to burn the program into K230, please read the K230 Quick Start tutorial first.

1. 

Bend the K230 module bracket to a suitable angle and connect the power supply. Place a brightly colored object in front of the K230 camera.
2. 

After K230 is initialized successfully, it will start to recognize the set color. After waiting for three seconds, the robot will start tracking.
...
![image-20250603171741439](https://www.yahboom.com/public/upload/upload-html/1749210422/image-20250603171741439.png)

Here you can change it to the color you want to track:

![image-20250603171841879](https://www.yahboom.com/public/upload/upload-html/1749210422/image-20250603171841879.png)
...
![image-20250530182310179](https://www.yahboom.com/public/upload/upload-html/1749210422/image-20250530182310179.png)
...
void Color_Trace(int x,int w,int h)
{
 ...
    if(pid_output>500) pid_output=500;//对pid输出值进行限幅，防止小车跑过头   Limit the pid output value to prevent the car from running too far
 ...
    if(pid_output1>200) pid_output1=200;//对pid输出值进行限幅，防止小车跑过头   Limit the pid output value to prevent the car from running too far
...
Color_Trace: After receiving the data from K230, it will wait for three seconds, then start calculating the x-axis offset and area offset of the recognized color, and finally control the robot.
...
![colortrack](https://www.yahboom.com/public/upload/upload-html/1749210422/colortrack.bmp)