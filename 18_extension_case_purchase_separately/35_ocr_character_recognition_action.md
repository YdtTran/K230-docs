# 14.OCR character recognition action

# OCR character recognition action
...
Place OCR_Rec_Action.py in the root directory of K230's sdcard and rename it to main.py, then reset or power on.
...
After K230 is initialized successfully, it will start to recognize characters and take corresponding actions based on the characters.

 

##### Notice:

Currently, the character recognition action only responds to the following six commands: "FORWARD", "BACKWARD", "LEFT", "RIGHT", "ROTATELEFT", and "ROTATERIGHT". All uppercase or all lowercase characters are supported.
...
OCRrec_Actions: When the recognized character content is "FORWARD", it moves forward for two seconds. When it is recognized as "BACKWARD", it moves back for two seconds. "LEFT" means turning left for two seconds, "RIGHT" means turning right for two seconds, "ROTATELEFT" means rotating left for two seconds, and "ROTATERIGHT" means rotating right for two seconds.
